...
$PREFIX docker buildx build --platform linux/amd64,linux/s390x,linux/arm64,linux/ppc64le --tag tetrate/grpcurl:${VERSION} --tag tetrate/grpcurl:latest --push --progress plain --no-cache .
$PREFIX docker buildx build --platform linux/amd64,linux/s390x,linux/arm64,linux/ppc64le --tag tetrate/grpcurl:${VERSION}-alpine --tag tetrate/grpcurl:latest-alpine --push --progress plain --no-cache --target alpine .
rm VERSION
